...
@section('content')
    ...
    <div class="d-flex align-items-center mb-md-3 mb-2">
        <div class="flex-fill">
            <div class="link">
                <img class="h-6 w-6 mr-2" style="min-width: 1.5rem;" src="https://www.google.com/s2/favicons?domain={{ $links->url }}" title="{{ $links->url }}"/>
                @if(isset(unserialize($links->rules)[1]))
                    <a title="{{ $links->name }}" href="{{ $links->url }}" class="btn">
                        @if(unserialize($links->rules)[1] == "any")
                            {{ $links->name }}
                        @else
                            {{ $links->name }}
                        @endif
                    </a>
                    <a data-url="@if(unserialize($links->rules)[1] == "any"){{ url('/')."/".$links->slug }}@else{{ unserialize($links->rules)[1]."/".$links->slug }}@endif" title="Copy Short URL" href="javascript:;" class="btn btn-outline-lime copy-btn">
                ...
                @else
                    ...
                    <a data-url="{{ route('link',['slug'=>$links->slug]) }}" title="Copy Short URL" href="javascript:;" class="btn btn-outline-lime copy-btn">
                        <i class="fas fa-copy"></i>
                    </a>                                            
                @endif
            </div>
    ...
    <form method="post" id="dateFilter" action="{{ route('link.dateFilter',['id'=>$links->id]) }}">
        @csrf
                ...
                <div class="card-body">
                    ...
                    <div class="row align-items-center mb-2">
                        <div class="col-7">
                            <h3 class="mb-0">
                                @if(isset($from) && isset($to))
                                    {{ count($links->click->whereBetween('created_at', [$from.' 00:00:00',$to.' 23:59:59'])) }}
                                @else
                                    {{ count($links->dateFilterClick) }}
                                @endif
                            </h3>
                        </div>
                        <div class="col-5">
                ...
                <div class="card-body">
                    ...
                    <div class="row align-items-center mb-2">
                        <div class="col-7">
                            <h3 class="mb-0">
                                @if(isset($from) && isset($to))
                                    {{ $links->click->where('is_first_click', 1)->whereBetween('created_at', [$from.' 00:00:00',$to.' 23:59:59'])->count() }}
                                @else
                                    {{ $links->dateFilterClick->where('is_first_click', 1)->count() }}
                                @endif
                            </h3>
                        </div>
                        <div class="col-5">
                ...
                <div class="card-body">
                    ...
                    <div class="row align-items-center mb-2">
                        <div class="col-7">
                            <h3 class="mb-0">
                                @if(isset($from) && isset($to))
                                    {{ $links->where('id', $links->id)->whereBetween('created_at', [$from.' 00:00:00',$to.' 23:59:59'])->count() }}
                                @else
                                    {{ $links->where('id', $links->id)->count() }}
                                @endif
                            </h3>
                        </div>
                        <div class="col-5">
                ...
                <div class="card-body">
                    ...
                    <div class="row align-items-center mb-2">
                        <div class="col-7">
                            <h3 class="mb-0">
                                @if(isset($from) && isset($to))
                                    {{ $links->linksGroup->where('link_id', $links->id)->count() }}
                                @else
                                    {{ $links->linksGroup->where('link_id', $links->id)->count() }}
                                @endif
                            </h3>
                ...
                <div class="card-body">
                    ...
                    <table id="clicks-details-table" class="table text-nowrap w-100" data-toggle="table"
                        ...
                        <tbody>
                                @if(isset($from) && isset($to))
                                    @php
                                        $linksClick = $links->click->whereBetween('created_at', [$from.' 00:00:00',$to.' 23:59:59']);
                                    @endphp
                                @else
                                    @php
                                    $linksClick = $links->dateFilterClick;
                                    @endphp
                                @endif
                            @foreach($linksClick as $key => $click)
                                <tr>
                                    <td>
    ...
    var handleRenderChartJs = function() {
        ...
        pieChart3 = new Chart(ctx4, {
            ...
            data: {
                ...
                datasets: [{
                    data: @if(isset($platforms['total'])) {!! json_encode($platforms['total']) !!} @else [] @endif,
                    //[116, 3, 3, 1],
                    backgroundColor: app_colors,
                    hoverBackgroundColor: app_colors,
                    borderWidth: 0
