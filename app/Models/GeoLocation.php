...
class GeoLocation {
	...
	public static function get_external_ip_address() {
		...
		// $transient_name = '';
		// if ( 'UNKNOWN' !== get_ip() ) {
		// 	$transient_name      = 'external_ip_address_' . get_ip();
		...
		// if ( false === $external_ip_address ) {
			...
			foreach ( $ip_lookup_services_keys as $service_name ) {
				...
				$response = get_curl_call($service_endpoint);
				if (!empty($response) && rest_is_ip_address($response)) {
					$external_ip_address = clean($response);
					break;
				}
			}
	...
	private static function geolocate_via_api( $ip_address ) {
		...
		// if ( false === $country_code ) {
			...
			foreach ( $geoip_services_keys as $service_name ) {
				...
				$response         = get_curl_call( sprintf( $service_endpoint, $ip_address ) );
				if (!empty($response)) {
					switch ( $service_name ) {
						case 'ipinfo.io':
							$country_code = isset( $response['country'] ) ? $response['country'] : '';
							break;
						case 'ip-api.com':
							$country_code = isset( $response['countryCode'] ) ? $response['countryCode'] : '';
							break;
						default:
							$country_code = '';
							break;
