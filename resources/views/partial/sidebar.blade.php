            ...
            @php
				...
				function renderSubMenu($value, $currentUrl) {
					...
					foreach ($value as $key => $menu) {
						...
						$menuIcon = (!empty($menu['icon'])) ? '<span class="menu-icon fontactive"><i class="'. $menu['icon'] .'"></i></span>' : '';
				...
				foreach (config('sidebar.menu') as $key => $menu) {
					...
					$menuText = (!empty($menu['text'])) ? '<span class="menu-text text-gray">'. $menu['text'] .'</span>' : '';
					$menuCaret = (!empty($hasSub)) ? '<span class="menu-caret"><b class="caret"></b></span>' : '';
					...
					if (!empty($menu['is_header'])) {
						...
						if(auth::user()->user_role == USER_ROLE_USER && 
							...
							{
							echo 	'<div class="menu-item mb-1 mt-1 '. $hasSub .' '. $active .'">
