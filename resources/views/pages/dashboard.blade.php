...
@section('content')
    ...
    <form method="post" id="dateFilter" action="{{ route('dashboard.dateFilter') }}">
        @csrf
    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="row align-items-center mb-2">
                        <div class="col-7">
                            <h3 class="mb-0">
                                <a href="javascript:;" class="text-inverse text-opacity-75 text-decoration-none">
                                    {{ $clicks->where('is_first_click', '=', 1)->count() }}
                                </a>
                            </h3>
                        </div>
                        {{-- <div class="col-5">
        ...
        <div class="col-xl-6">
            <div class="card mb-3">
                <div class="card-body">
                    <div class="d-flex fw-bold small mb-3">
                        <span class="flex-grow-1">{{ $clicks->count() }} TOTAL CLICKS</span>
                        <a href="#" data-toggle="card-expand"
                            class="text-inverse text-opacity-50 text-decoration-none"><i class="bi bi-fullscreen"></i></a>
                    </div>
...
@push('js')
    ...
    <script>
        ...
        var handleRenderChartJs = function() {
            var app_colors = [];
            let getCookies = Cookies.get("app-theme-mode");
            $.each(app.color, (name, color) => {
                if (!name.endsWith("Rgb") && !app_colors.includes(color)) {
                    {{-- if(color == "#FFFFFF" && getCookies == 'light'){
